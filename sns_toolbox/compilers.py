...
def __compile_numpy__(network, dt=0.01, debug=False) -> SNS_Numpy:
    ...
    params = {'dt': dt,
              ...
              'gM': g_m.tolist(),
              'iB': i_b.tolist(),
              'gMaxNon': g_max_non.tolist(),
              'delE': del_e.tolist(),
              'eLo': e_lo.tolist(),
              'eHi': e_hi.tolist(),
              'timeFactorMembrane': time_factor_membrane.tolist(),
              'inputConn': input_connectivity.tolist(),
              ...
              # 'r': R,
              'outConnVolt': output_voltage_connectivity.tolist()}
    if spiking:
        params['spikes'] = spikes.tolist()
        params['theta0'] = theta_0.tolist()
        params['theta'] = theta.tolist()
        params['thetaLast'] = theta_last.tolist()
        params['m'] = m.tolist()
        params['tauTheta'] = tau_theta.tolist()
        params['gMaxSpike'] = g_max_spike.tolist()
        params['gSpike'] = g_spike.tolist()
        params['tauSyn'] = tau_syn.tolist()
        params['timeFactorThreshold'] = time_factor_threshold.tolist()
        params['timeFactorSynapse'] = time_factor_synapse.tolist()
        params['outConnSpike'] = output_spike_connectivity.tolist()
        params['thetaLeak'] = theta_leak.tolist()
        params['thetaIncrement'] = theta_increment.tolist()
        params['thetaFloor'] = theta_floor.tolist()
        params['vReset'] = V_reset.tolist()
        params['gIncrement'] = g_increment.tolist()
    if delay:
        params['spikeDelays'] = spike_delays.tolist()
        params['spikeRows'] = spike_rows
        params['spikeCols'] = spike_cols
        params['bufferSteps'] = buffer_steps
        params['bufferNrns'] = buffer_nrns
        params['delayedSpikes'] = delayed_spikes.tolist()
        params['spikeBuffer'] = spike_buffer.tolist()
    if electrical:
        params['gElectrical'] = g_electrical.tolist()
    if electrical_rectified:
        params['gRectified'] = g_rectified.tolist()
    if gated:
        params['gIon'] = g_ion.tolist()
        params['eIon'] = e_ion.tolist()
        params['powA'] = pow_a.tolist()
        params['slopeA'] = slope_a.tolist()
        params['kA'] = k_a.tolist()
        params['eA'] = e_a.tolist()
        params['powB'] = pow_b.tolist()
        params['slopeB'] = slope_b.tolist()
        params['kB'] = k_b.tolist()
        params['eB'] = e_b.tolist()
        params['tauMaxB'] = tau_max_b.tolist()
        params['powC'] = pow_c.tolist()
        params['slopeC'] = slope_c.tolist()
        params['kC'] = k_c.tolist()
        params['eC'] = e_c.tolist()
        params['tauMaxC'] = tau_max_c.tolist()
        params['bGate'] = b_gate.tolist()
        params['bGateLast'] = b_gate_last.tolist()
        params['bGate0'] = b_gate_0.tolist()
        params['cGate'] = c_gate.tolist()
        params['cGateLast'] = c_gate_last.tolist()
        params['cGate0'] = c_gate_0.tolist()
